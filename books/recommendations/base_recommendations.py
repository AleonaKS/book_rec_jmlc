...
def recommendations_for_anonymous(session_key, top_n=20):
    ...
    exclude_ids = {b.id for b in popular_books + new_books + soon_books}
    recommended_books = list(Book.objects.filter(new=False, soon=False).exclude(id__in=exclude_ids).order_by('-rating_chitai_gorod')[:top_n])
    return popular_books[:top_n], new_books[:top_n], soon_books[:top_n], recommended_books
